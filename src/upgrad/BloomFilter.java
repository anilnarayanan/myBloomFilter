...
//		}
		...
		hash1 = Math.abs(chf.genHash(x));
		hash2 = Math.abs(chf.genHash2(x));
//        hash3 = Math.abs(chf.genHash3(x)%10);
...
//			bs.set(Math.abs((hash1) % size));
//			bs.set(Math.abs((hash2) % size));
//			bs.set(Math.abs((hash3) % size));
		}
	}
